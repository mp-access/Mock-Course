...
def preview_dognames(list, n):
    ...
    A call 'preview_dognames(list, 4)' should return the list ['Adam', 'Berta', 'Collin', 'Eve']
...
def dognames_count(list):
    ...
    names_count = {}
    for row in list:
        if row['HUNDENAME'] not in names_count.keys():
            names_count[row['HUNDENAME']] = 1
        else:
            names_count[row['HUNDENAME']] += 1
...
def top_n_dognames(list, n):
    ...
    names_count = dognames_count(list)
    sorted_names_count = sorted(names_count.items(), key=operator.itemgetter(1), reverse=True)

    return sorted_names_count[:n]
...
def is_valid_row(row, year=None, sex=None):
    ...
    is_valid_row(row, year=None, sex=None) --> no filter specified, this should always return True
    is_valid_row(row, year=None, sex='w') --> should return False (wrong sex)
    ...
    if year and not int(row['GEBURTSJAHR_HUND']) == year:
        return False
    elif sex and not row['GESCHLECHT_HUND'] == sex:
        return False
...
def filter_dognames(list, year=None, sex=None):
    """
    Returns a filtered list with entries of type OrderedDict, containing all entries from the given list that satisfy
    the conditions. Note that you don't have to change the type of the rows that you get from the list, you only need
    ...
    :param year: An integer parameter specifying the birth year. Any integer is accepted, or None if no filter should be applied.
    :param sex: A string parameter specifying the sex. Possible values: 'm', 'w' or None if no filter should be applied.
    :return: A list containing all entries of the list that satisfy the conditions.
    """
    filtered_dognames = []
    for row in list:
        if is_valid_row(row, year, sex):
            filtered_dognames.append(row)

    return filtered_dognames
